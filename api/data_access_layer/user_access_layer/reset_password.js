...
async function resetPassword(email, callback){
  try{
    ...
    const data = {
      ...
      text:`HI, ${email} a password reset has been initialized on your account with us. if you are the one that initialised it click on this link below else ignore this message ${url}/${email}/${token} `
    }
    emailer(user.email, data, async (err, result) => {
      if (err){
        return callback(err)
      }
      const update = await Users.findOneAndUpdate({email},{$set:{
        authToken: token
      }}, {new: true});
      if (!update){
        const err = {
